...
public class ilk_DamageUtilsTest {
  ...
  public void testGetDamageAtHitStrengthHighArmor() {
    ...
    final float modifiedDamage = ilk_DamageUtils.getDamageAtHitStrength(baseDamage, DamageType.ENERGY, strength, armor);

    assertThat(modifiedDamage).isAtLeast(baseDamage);
    assertThat(applyArmorReduction(baseDamage, strength, armor)).isWithin(.01f)
  ...
  public void testGetDamageAtHitStrengthHeDamage() {
    ...
    final float modifiedDamage = ilk_DamageUtils.getDamageAtHitStrength(baseDamage, DamageType.HIGH_EXPLOSIVE, strength,
        armor);

    assertThat(modifiedDamage).isAtLeast(baseDamage);
    assertThat(applyArmorReduction(2 * baseDamage, 2 * strength, armor)).isWithin(.01f)
  ...
  public void testGetDamageAtHitStrengthKineticDamage() {
    ...
    final float strength = 100;
    final float armor = 50;
    final float modifiedDamage = ilk_DamageUtils.getDamageAtHitStrength(baseDamage, DamageType.KINETIC, strength,
        armor);

    assertThat(modifiedDamage).isAtLeast(baseDamage);
    assertThat(applyArmorReduction(0.5f * baseDamage, 0.5f * strength, armor)).isWithin(.01f)
