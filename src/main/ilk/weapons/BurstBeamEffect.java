...
import ilk.util.Interpolation;

public class BurstBeamEffect implements BeamEffectPlugin {
  ...
  public void beamSpike(BeamAPI beamAPI, float delta) {
    if (beamAPI.getWeapon().isFiring()) {
      ...
      beamAPI.setWidth(
          Interpolation.linear(Interpolation.clamp(amplitude)) * (SPIKE_MULT_VAL - 1f) * startingWidth + startingWidth);
    } else {
      duration = 0f;
    }
  }
