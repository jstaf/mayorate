...
public class CollisionUtilsEX {
  ...
  // ship hit! return the collision point of segment segStart to segEnd and a ship
  // (will consider shield).
    ...
    if (shield == null || shield.isOff()) {
      ...
      if (tmp1 != null) {
        ...
        if (shield.isWithinArc(tmp1)) {
          ...
          // if the hit come outside the shield's arc but it hit the shield's "edge", find
          // that
          // point.
          boolean hit = false;
          ...
          Vector2f shieldEdge1 = MathUtils.getPointOnCircumference(circleCenter, circleRadius,
          ...
          Vector2f shieldEdge2 = MathUtils.getPointOnCircumference(circleCenter, circleRadius,
              MathUtils.clampAngle(shield.getFacing() - shield.getActiveArc() / 2));
  ...
  ///////////////////////////////////////// circumference.
  // if segStart is outside the circle and segment can not intersection with the
  ///////////////////////////////////////// circumference, will
  // return null.
  // if segStart is inside the circle, will return segStart.
  public static Vector2f getCollisionPointOnCircumference(Vector2f segStart, Vector2f segEnd, Vector2f circleCenter,
      float circleRadius) {

    Vector2f startToEnd = Vector2f.sub(segEnd, segStart, null);
    Vector2f startToCenter = Vector2f.sub(circleCenter, segStart, null);
    double ptLineDistSq = (float) Line2D.ptLineDistSq(segStart.x, segStart.y, segEnd.x, segEnd.y, circleCenter.x,
        circleCenter.y);
    ...
    // if lineStart is outside the circle and segment can not reach the
    // circumference, return null
  ...
  ///////////////////////////////////////// getShipCollisionPoint
  // instead.
  // return the collision point of segment segStart to segEnd and ship's shield.
  // if the segment can not hit the shield or if the ship has no shield, return
  ///////////////////////////////////////// null.
