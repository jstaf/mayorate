...
public class ilk_FighterCoprocessingAi implements ShipSystemAIScript {

  private Logger logger = Global.getLogger(ilk_FighterCoprocessingAi.class);

  // The range at which a fighter under regroup orders will be considered regrouping.
  private final float REGROUP_RANGE = 500.0f;
  // The range within which enemy ships and missiles will be considered a threat to the carrier.
  private final float THREAT_RANGE = 1000.0f;
  // How much threat missiles add.
  private final float THREAT_PER_MISSILE_DAMAGE = 0.001f;
  // The threat at which the carrier will deactivate iff its wings are not themselves engaging an
  // enemy.
  ...
  public void advance(
    ...
    // Deactivate If the ship AI does not want to use flux, or this is the only thing standing in
    ...
    if (flags.hasFlag(AIFlags.DO_NOT_USE_FLUX)
        || (ship.getMutableStats().getZeroFluxMinimumFluxLevel().getModifiedValue() == 0.0f
                && (ship.getShield() == null || ship.getShield().isOff()))
    ...
    // Deactivate if fighters are regrouping. This checks the fighters' distance as well as their
    // orders because the mobility boost can help distant fighters survive the return to the
    // carrier.
    ...
    for (ShipAPI fighter : fighters) {
      maxFighterRange =
          Math.max(
              maxFighterRange, MathUtils.getDistance(ship.getLocation(), fighter.getLocation()));
    }
    ...
    if (threat >= BASE_THREAT_THRESHOLD) {
      ...
      if (!fightersEngaged) {
        logger.debug(
            "Deactivating because the carrier is under mild threat and fighters are not engaged.");
        deactivate();
        return;
      }
    }

    // Having got this far, we know fighters are out and the carrier is not under threat--activate!
    logger.debug("Activating system.");
    activate();
  }

  // Toggles the system if it is inactive, or does nothing if it is already active.
  ...
  // Toggles the system if it is active, or does nothing if it is already inactive.
