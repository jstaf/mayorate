...
public class ilk_CollisionUtilsEX {
  ...
  public static Vector2f getShipCollisionPoint(Vector2f segStart, Vector2f segEnd, ShipAPI ship) {
    ...
    if (shield == null || shield.isOff()) {
      ...
      Vector2f tmp1 =
          getCollisionPointOnCircumference(segStart, segEnd, circleCenter, circleRadius);
      if (tmp1 != null) {
        // OK! hit the shield in face
        if (shield.isWithinArc(tmp1)) {
          return tmp1;
        } else {
          // if the hit come outside the shield's arc but it hit the shield's "edge", find that
          // point.
          boolean hit = false;
          ...
          Vector2f shieldEdge1 =
              MathUtils.getPointOnCircumference(
                  circleCenter,
                  circleRadius,
                  MathUtils.clampAngle(shield.getFacing() + shield.getActiveArc() / 2));
          Vector2f tmp2 =
              CollisionUtils.getCollisionPoint(segStart, tmp, circleCenter, shieldEdge1);
          if (tmp2 != null) {
            tmp = tmp2;
            hit = true;
          }
          Vector2f shieldEdge2 =
              MathUtils.getPointOnCircumference(
                  ...
                  MathUtils.clampAngle(shield.getFacing() - shield.getActiveArc() / 2));
          Vector2f tmp3 =
              CollisionUtils.getCollisionPoint(segStart, tmp, circleCenter, shieldEdge2);
          if (tmp3 != null) {
            tmp = tmp3;
            hit = true;
          }
  ...
  // if segStart is outside the circle and segment can not intersection with the circumference, will
  // return null.
  // if segStart is inside the circle, will return segStart.
  public static Vector2f getCollisionPointOnCircumference(
      Vector2f segStart, Vector2f segEnd, Vector2f circleCenter, float circleRadius) {
    ...
    double dist =
        Vector2f.dot(startToCenter, startToEnd) - Math.sqrt(circleRadiusSq - ptLineDistSq);
  ...
  // if you need the check for a ship hit (considering it's shield), use getShipCollisionPoint
  // instead.
  ...
  public static Vector2f getShieldCollisionPoint(
      Vector2f segStart, Vector2f segEnd, ShipAPI ship, boolean ignoreHull) {
