...
public class ilk_NukeOnHitEffect implements OnHitEffectPlugin {
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit,
        ...
        Global.getCombatEngine().spawnProjectile(projectile.getSource(), projectile.getWeapon(), "ilk_nuke_primaryDet",
                point, 0f, null);
        Global.getCombatEngine().applyDamage(projectile, point, projectile.getHitpoints() * 100f,
                DamageType.FRAGMENTATION, 0f, false, false, projectile);
        for (int i = 0; i < 30; i++) {
            float angle = (float) (i - 1) * 12f;
            Vector2f location = MathUtils.getPointOnCircumference(projectile.getLocation(), 10f, angle);
            Global.getCombatEngine().spawnProjectile(projectile.getSource(), projectile.getWeapon(), "ilk_nuke_damage",
                    location, angle, null);
        }
    }
}
