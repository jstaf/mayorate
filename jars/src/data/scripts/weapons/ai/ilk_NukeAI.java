...
public class ilk_NukeAI implements MissileAIPlugin, GuidedMissileAI {
    ...
    public static void detonate(MissileAPI missile) {
        ...
        for (int i = 0; i < 60; i++) {
            ...
            Global.getCombatEngine().spawnProjectile(missile.getSource(), missile.getWeapon(), "ilk_nuke_damage", location, angle, null);
        }
    }
    ...
    public void advance(float amount) {
        ...
        if (target == null || (target instanceof ShipAPI && (((ShipAPI) target).isHulk())) || target.getCollisionClass() == CollisionClass.NONE || (missile.getOwner() == target.getOwner()) || !Global.getCombatEngine().isEntityInPlay(target)) {
            setTarget(findBestTarget(missile));
            if (target == null) {
