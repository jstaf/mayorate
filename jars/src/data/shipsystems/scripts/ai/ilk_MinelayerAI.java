...
public class ilk_MinelayerAI implements ShipSystemAIScript {
    ...
    public void advance(float v, Vector2f vector2f, Vector2f vector2f1, ShipAPI shipAPI) {
        ...
        if (ship.isRetreating()) {
            // deploy mines if there's an enemy anywhere within sight! (hopefully this will slow them the fuck down.)
        ...
        if (orders != null) {
            if (orders.getType().equals(CombatAssignmentType.DEFEND) || orders.getType().equals(CombatAssignmentType.CONTROL)){
